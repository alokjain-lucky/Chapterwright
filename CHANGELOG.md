...
- **Fixed book/chapter meta silently failing to save**, at all, through any code path — `_mab_accent`, `_mab_subtitle`, `_mab_book_id`, `_mab_order`, `_mab_section_id` and every other registered meta field. Root cause: neither `mab_book` nor `mab_chapter`'s `supports` array (`mab_register_post_types()`, `includes/content-types.php`) included `'custom-fields'`, which `WP_REST_Posts_Controller::get_item_schema()` requires before it will add a `meta` property to a post type's REST schema at all. `register_post_meta()` still ran and the keys were genuinely registered, so nothing looked broken from that side — every `meta` value sent via the REST API was just silently accepted and silently discarded, with no error and no rejected write. Books "worked" only by coincidence: `_mab_accent`'s sanitize callback happens to fall back to a real-looking color (`#f45d48`) on invalid input, masking the bug. Fixed by adding `'custom-fields'` to both `supports` arrays. One pre-existing chapter (ID 59) created before this fix remains orphaned (no `_mab_book_id`) and was intentionally left untouched rather than deleted, since it has real content.
...
- **Added a "coming soon" flag for books**: new `_mab_coming_soon` meta on `mab_book`. Checking it in the admin app (`BookFields`, `admin/app/src/screens/book-detail.js`) auto-publishes an unpublished book in the same save request, since the admin app had no separate publish control; unchecking it never un-publishes. Front-end: `templates/book-grid.php` swaps the archive card's CTA and adds a "Coming soon" badge; `templates/single-mab_book.php` shows the same badge and hides "Start reading".
- **Added a `show_draft_chapters` setting** (off by default) to optionally surface draft chapter titles in the table of contents. `mab_get_chapters()` (`includes/queries.php`) gained an optional `$statuses` param so TOC rendering can fetch `array('publish','draft')` without affecting the `$chapters` variable that drives prev/next navigation and pagination — those never include drafts. A draft chapter renders as a non-clickable, dimmed row (`.mab-toc-row-link--draft`) instead of a link, since its permalink would 404 or leak a preview URL for a logged-out visitor.
- **Added a book-completion progress bar** on the chapter reader/nav dividers — a gradient bar showing how far into the whole book the reader is (distinct from the existing scroll-position progress bar for the current chapter), computed in `templates/single-mab_chapter.php` and exposed as a `--mab-progress` custom property.
- **Fixed three CSS specificity bugs**, all found by live-testing rendered output and matching `document.styleSheets` rules against the actual DOM rather than by re-reading source: (1) `wp-content/themes/alok-jain/assets/css/books.css`'s light-theme block was missing a `--mab-code-ink` override, leaving code text dark-on-dark in light mode; (2) `.mab-toc-jump`'s own `color` lost to the `.mab-page a { color: inherit; }` reset; (3) `.mab-book-card__cover > span { color: #fff; }` (meant for the no-cover-image title placeholder) also matched the new "Coming soon" badge sitting alongside it, rendering it invisible — fixed by giving the placeholder its own dedicated class (`.mab-book-card__cover-label`) rather than adding more specificity.
